...
* Открыть текстовый файл по следующему пути относительно корневой директории проекта: <some-prjoect-dir>/test/CMakeLists.txt и изменить строку
    ...
    Изменив путь на верный путь к директории QT. Путь должен до qmake.exe.
...
Далее необходимо открыть Spam_Detectpr.sln и выполнить сборку стандартным для Visual Studio способом. Для запуска unit-test существует проект, накодящийся в папке "test" основного проекта.
## Запуск продукта
